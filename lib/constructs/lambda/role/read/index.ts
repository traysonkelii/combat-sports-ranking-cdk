...
export interface GetUsersByRoleLambdaProps {
  table: ITable;
  region: string;
  layers?: lambda.ILayerVersion[];
}

export class GetUsersByRoleLambda extends Construct {
  readonly lambda: lambda.IFunction;
  constructor(
    scope: Construct,
    id: string,
    { table, region, layers }: GetUsersByRoleLambdaProps
  ) {
    super(scope, id);
    ...
    this.lambda = new LambdaBase(this, id, {
      table,
      region,
      handler: "index.getUsersByRoleHandler",
