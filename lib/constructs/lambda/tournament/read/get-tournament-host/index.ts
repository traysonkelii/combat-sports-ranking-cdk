...
export interface GetTournamentHostLambdaProps {
  table: ITable;
  region: string;
  layers?: lambda.ILayerVersion[];
}

export class GetTournamentHostLambda extends Construct {
  readonly lambda: lambda.IFunction;
  constructor(
    scope: Construct,
    id: string,
    { table, region, layers }: GetTournamentHostLambdaProps
  ) {
    super(scope, id);
    ...
    this.lambda = new LambdaBase(this, id, {
      table,
      region,
      handler: "index.getTournamentHostHander",
